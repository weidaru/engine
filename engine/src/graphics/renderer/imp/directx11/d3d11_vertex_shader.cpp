...
bool D3D11VertexShader::Initialize(ShaderBytecode *_bytecode) {
	...
	HRESULT result = 1;
	result = manager->GetDevice()->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), 0, &shader);
	if(FAILED(result)) {
		S2StringFormat(&error, "Cannot create vertex shader. Error code %d", ::GetLastError());
		return false;
	} else {
		return true;
	}
...
bool D3D11VertexShader::Initialize(const s2string &path, const s2string &entry_point) {
	...
	if( b->Initialize(path, entry_point, ShaderType::VERTEX) == false ) {
