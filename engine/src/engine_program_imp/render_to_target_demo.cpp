...
public:
	...
	void CreateColorProgram() {
		...
		normal_state->SetPrimitiveTopology(GraphicPipeline::TRIANGLE_LIST);
		normal_state->SetVertexShader(vs);
		normal_state->SetVertexShaderData(vs_data);
		normal_state->SetPixelShader(ps);
		normal_state->SetDepthStencil(ds_buffer->AsDepthStencil());
		normal_state->SetRenderTarget(0, Engine::GetSingleton()->GetRendererContext()->GetBackBuffer()->AsRenderTarget());
		normal_state->SetRenderTarget(1, texture->AsRenderTarget());
		normal_state->SetVertexBuffer(0, vb->AsVertexBuffer());
		normal_state->SetIndexBuffer(ib->AsIndexBuffer());
	...
	void CreateTextureProgram() {
		...
		tex_state->SetPrimitiveTopology(GraphicPipeline::TRIANGLE_LIST);
		tex_state->SetRenderTarget(1, 0);
		tex_state->SetDepthStencil(ds_buffer->AsDepthStencil());
		tex_state->SetVertexShader(tex_vs);
		tex_state->SetPixelShader(tex_ps);
		tex_state->SetPixelShaderData(tex_ps_data);
		tex_state->SetRenderTarget(0, Engine::GetSingleton()->GetRendererContext()->GetBackBuffer()->AsRenderTarget());
		tex_state->SetVertexBuffer(0, tex_vb->AsVertexBuffer());
		tex_state->SetIndexBuffer(tex_ib->AsIndexBuffer());
